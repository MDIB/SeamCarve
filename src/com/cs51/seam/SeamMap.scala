...
/** A Node representing a pixel
 ...
 case class Node(val x: Int, val y: Int, val energy: Double, val color: Int) extends NodeItem {
...
private object Factory {
	 ...
	 */
	def getNeighbors(emap : EnergyMap, img : BufferedImage) : (HashMap[Node, Neighbors], Node) = {
	  //get node links
	  
	  //TODO: provide check to make sure they are of similar size
		val nodes = Array.ofDim[Node](img.getWidth, emap.getHeight)
		for (x <- 0 to (emap.getWidth() - 1); y <- 0 to (emap.getHeight() - 1)) {
			nodes(x)(y) = new Node(x,y,emap.getEnergy(x, y), img.getRGB(x, y))
		}
 ...
 class SeamMap private(neighbormap : (HashMap[Node, Neighbors], Node)) {
   ...
   def this(emap : EnergyMap, img : BufferedImage) = this(Factory.getNeighbors(emap : EnergyMap, img : BufferedImage))
	 ...
	 */
	def drawSeam(nodes : List[Node], img : BufferedImage) : BufferedImage = {
	  def drawPixel(nodes : List[Node], img: BufferedImage) : Unit = {
		  nodes match {
		    case node::tl => 
		    	img.setRGB(node.x, node.y, Utils.getIntFromRGB(255,0,0));
		    	drawPixel(tl, img)
		    case Nil => ()
		  }
	  }
	  
	  //concept from here:  http://stackoverflow.com/questions/3514158/how-do-you-clone-a-bufferedimage
	  val cm = img.getColorModel
	  val newImg = new BufferedImage(cm, img.copyData(null), cm.isAlphaPremultiplied, null);
	...
	def getSeamInsertionImage(vertSeams : List[Node], horzSeams : List[Node]) : BufferedImage = {
	  ...
	  def determineHash(vertSeams : List[Node], horzSeams : List[Node], doubleXY : HashMap[Node, (Boolean, Boolean)]) : HashMap[Node, (Boolean, Boolean)] = {
  	  ...
  	  def setRow(node : Node, x : Int,  Dstruct: Array[List[Int]], doubleXY : HashMap[Node, (Boolean, Boolean)]) 
  	  		: Unit = {
	  ...
	  def setRows(leftNode : Node, Dstruct: Array[List[Int]], doubleXY : HashMap[Node, (Boolean, Boolean)]) : Unit = {
	  ...
	  def doCheck(Dstruct : Array[List[Int]], vertSize : Int, curX : Int) : Unit = {
	  ...
	  def writeToImage(img : BufferedImage, curList : List[Int], curY : Int, height : Int, curX : Int, DStruct : Array[List[Int]]) : Unit = {
	    curList match {
	      case hd::tl => img.setRGB(curX, curY, hd); writeToImage(img, tl, (curY - 1), height, curX, DStruct)
	      case Nil => writeToImage(img, DStruct(curX+1), height, height, curX+1, DStruct)
	    }
	  }
	  
	  //will hold pixel color while determining
	  val Dstruct = new Array[List[Int]](getWidth(upperLeft, 0) + vertSeams.length)
	  ...
	  val img = new BufferedImage(Dstruct.length, height, BufferedImage.TYPE_3BYTE_BGR)
