...
class SeamCarve (img : BufferedImage, maxEnergy : Double, 
	...
	def getAnimPics : Array[File] = {
	  ...
	  val (animpics, seampic) = 
	    revOrderSeams.reverse.foldLeft((Array[File](), copyImage(img))) {
	        (arrAndImage, seam) => 
	        	val (curArray, curImage) = arrAndImage

		        //get image with seam drawn on it
		        val nextImage = drawSeamNodes(copyImage(curImage), seam, (Color.red).getRGB())
	  ...
	  println("reverse order seams")
	  //show seams being drawn

	  
	  val (_, seamRemoval) = revOrderSeams.reverse.foldLeft( (graphWithSeams, Array[File]() ) ) {
	    (graphAndArray, seam) =>
	        	...
	        	Utils.combineArrays(theArray,
	        		Array[File](
	            		Utils.imageToTempFile(
	            		    Utils.giveEdges(newGraph.getImage, Color.black,
	            		        img.getHeight, img.getWidth, Utils.ImgPosition.Center)))))
	  }
